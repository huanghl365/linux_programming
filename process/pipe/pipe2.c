...
/*
程序功能描述：创建管道，关闭管道写端和读端，观察从管道读端read的现象
*/

/*
关于关闭管道写端和读端后，执行read的结果
1、只关闭父进程的管道写端，子进程还是会阻塞等待，只有同时关闭父子进程的写端，
管道才会真正无法写入数据，这样read才会返回0。
2、如果关闭父进程的读端，然后从父进程的读端读取数据，这时就会报错，返回-1.
*/
...
int main(void)
{
	...
	*/
	...
	if (pid > 0)
	{
		...
		/*
		测试关闭管道写端之后，对read的影响
		测试结果：只关闭父进程的管道写端，子进程还是会阻塞等待，只有同时关闭父子进程的写端，
		管道才会真正无法写入数据，这样read才会返回0。
